...
-- =========================================================================
-- Generated on: 2025-06-25 21:37:31 UTC
-- Environment: dev
...
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_BRONZE_USER_OVERVIEW;

-- View top 10 countries by user count
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_TOP_COUNTRIES LIMIT 10;

-- Check data quality distribution
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_DATA_QUALITY_DISTRIBUTION;

-- View email provider market share
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_EMAIL_PROVIDER_MARKET_SHARE;
...
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_MONTHLY_REGISTRATION_TRENDS LIMIT 12;

-- View quarterly performance
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_QUARTERLY_REGISTRATION_PERFORMANCE;

-- Check day of week patterns
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_REGISTRATION_DAY_PATTERNS;

-- View top growing countries
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_TOP_GROWING_COUNTRIES LIMIT 10;

-- Check data quality trends
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_DATA_QUALITY_TREND_OVERVIEW LIMIT 10;

-- View anomaly alerts
SELECT * FROM ECOMMERCE_DB.GOLD_LAYER.VW_ANOMALY_ALERT_DASHBOARD;
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_TARGET_SEGMENT_DISTRIBUTION demo
LEFT JOIN ECOMMERCE_DB.GOLD_LAYER.VW_DATA_QUALITY_DISTRIBUTION qual
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_TOP_COUNTRIES geo
LEFT JOIN ECOMMERCE_DB.GOLD_LAYER.VW_ENGAGEMENT_BY_GENERATION eng
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_EMAIL_PROVIDER_MARKET_SHARE emp
JOIN ECOMMERCE_DB.GOLD_LAYER.VW_EMAIL_PROVIDER_AGE_DEMOGRAPHICS age_demo
...
SELECT 
    ...
    ts.growth_rate,
    ts.processing_date
FROM ECOMMERCE_DB.GOLD_LAYER.VW_TIME_SERIES_EXECUTIVE_DASHBOARD ts
...
SELECT 
    ...
    earliest_entry_date,
    latest_entry_date
FROM ECOMMERCE_DB.GOLD_LAYER.VW_GEOGRAPHIC_EXPANSION_INSIGHTS
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_AGE_GROUP_TRENDS
...
SELECT 
    ...
    'users' as unit
FROM ECOMMERCE_DB.GOLD_LAYER.VW_EXECUTIVE_SUMMARY
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_DATA_QUALITY_BATCH_DISTRIBUTION
...
LEFT JOIN (
    ...
    FROM ECOMMERCE_DB.GOLD_LAYER.VW_CITIES_BUSINESS_EMAIL_CONCENTRATION
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_GENDER_BY_AGE_GROUP age
LEFT JOIN ECOMMERCE_DB.GOLD_LAYER.VW_ENGAGEMENT_BY_GENERATION eng
    ON eng.engagement_tier = 'High'
...
SELECT 
    ...
    quality_direction,
    overall_performance
FROM ECOMMERCE_DB.GOLD_LAYER.VW_MONTHLY_BUSINESS_SUMMARY
ORDER BY month DESC
LIMIT 12;
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_COUNTRY_EXPANSION_TIMELINE
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_GENERATION_PERFORMANCE_TIMELINE
...
SELECT 
    ...
    CASE 
        ...
        ELSE 'OK'
    END as alert_status
FROM ECOMMERCE_DB.GOLD_LAYER.VW_BRONZE_VS_SILVER_COMPLETENESS;
...
SELECT 
    ...
    action_required,
    processing_date
FROM ECOMMERCE_DB.GOLD_LAYER.VW_REGISTRATION_ANOMALIES_SUMMARY
...
SELECT 
    ...
    quality_status,
    total_records
FROM ECOMMERCE_DB.GOLD_LAYER.VW_QUALITY_ANOMALIES_SUMMARY
WHERE anomaly_severity IN ('Critical', 'High')
ORDER BY z_score DESC, processing_date DESC;
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_QUALITY_IMPROVEMENT_TRACKING
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_MONTHLY_REGISTRATION_TRENDS
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_MONTHLY_REGISTRATION_TRENDS rt
LEFT JOIN ECOMMERCE_DB.GOLD_LAYER.VW_DATA_QUALITY_TREND_OVERVIEW qt
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_FASTEST_GROWING_DEMOGRAPHICS demo
LEFT JOIN ECOMMERCE_DB.GOLD_LAYER.VW_GEOGRAPHIC_EXPANSION_INSIGHTS geo
...
FROM ECOMMERCE_DB.GOLD_LAYER.VW_MARKET_PENETRATION_ANALYSIS geo
CROSS JOIN ECOMMERCE_DB.GOLD_LAYER.VW_REGISTRATION_DAY_PATTERNS patterns
...
LEFT JOIN ECOMMERCE_DB.SILVER_LAYER.FACT_GEOGRAPHIC_EXPANSION ge
    ON rt.processing_date = ge.processing_date
LEFT JOIN ECOMMERCE_DB.SILVER_LAYER.FACT_DATA_QUALITY_TRENDS qt
    ON rt.processing_date = qt.processing_date
LEFT JOIN ECOMMERCE_DB.SILVER_LAYER.FACT_REGISTRATION_ANOMALIES ra
    ON rt.processing_date = ra.processing_date AND ra.is_anomaly = TRUE
LEFT JOIN ECOMMERCE_DB.SILVER_LAYER.FACT_QUALITY_ANOMALIES qa
...
SHOW VIEWS IN SCHEMA ECOMMERCE_DB.GOLD_LAYER;
...
SELECT 
    ...
    COUNT(*) as view_count
FROM (SHOW VIEWS IN SCHEMA ECOMMERCE_DB.GOLD_LAYER)
GROUP BY view_category
ORDER BY view_count DESC;

-- Get view definitions (useful for documentation)
-- Note: Replace view name as needed
-- SHOW CREATE VIEW ECOMMERCE_DB.GOLD_LAYER.VW_TIME_SERIES_EXECUTIVE_DASHBOARD;
